...
interface ProcessoOrcamento {
  ...
  semMedico?: boolean;
  /** #708 (@R 24/09): pedido de exame e o retorno dele (back exame_solicitado.py) — null = não há pedido de exame */
...
/** #708: o selo do pedido de exame na linha do paciente. Vermelho = o e-mail não saiu (ninguém vai responder o que não
 *  chegou); azul = saiu e espera; verde = o retorno chegou e ninguém viu ainda (botão "vi"); cinza = retorno já visto. */
function SeloExame({ r, onVisto }: { r: ProcessoOrcamento; onVisto: () => void }) {
...
[TEL] Qualquer duvida, entrar em contato com a equipe G4Med.`
  ...
  return (
    <div className="orcamento-medico-page">
      ...
      {(() => {
        // #708: o aviso do topo — retornos de exame que chegaram e ninguém viu, e pedidos de exame cujo e-mail não saiu
        const comExame = dataComMedico.filter((p: ProcessoOrcamento) => p.exameSolicitado);
        const chegaram = comExame.filter((p: ProcessoOrcamento) => p.exameSolicitado!.estado === 'RETORNO' && p.exameSolicitado!.retornosNaoVistos > 0);
        const naoSairam = comExame.filter((p: ProcessoOrcamento) => p.exameSolicitado!.estado === 'NAO_SAIU');
        if (!chegaram.length && !naoSairam.length) return null;
        const lista = (ps: ProcessoOrcamento[]) => ps.map((p) => `#${p.id} ${p.paciente}`).join(' · ');
        return (
          <div className="aviso-exames" role="status">
            ...
            {naoSairam.length > 0 && (
              <div className="aviso-exames__linha aviso-exames__linha--nao-saiu">
                ...
                <a href="/emails" onClick={(e) => { e.preventDefault(); navigate('/emails'); }}>E-mails pendentes</a>: {lista(naoSairam)}
              </div>
            )}
          </div>
        );
